...
Public Function CheckUpdate(TmpFile As String, lblOut As Label) As String
    TampilkanBalon frmMain, j_bahasa(26), i_bahasa(26), NIIF_INFO
    lblOut.Caption = j_bahasa(33)
    If ValidFile(TmpFile) = True Then
       DoEvents
       If AdakahUpdateUntukVersiDipakai(TmpFile) = True Then
          CheckUpdate = PenampungInfoUpdate & ". - [Kabar Baru Web CMC] " & PenampungBerita
          bUpdateCompon = True
          TampilkanBalon frmMain, PenampungInfoUpdate, i_bahasa(26), NIIF_INFO
       Else
          bUpdateCompon = False
          CheckUpdate = j_bahasa(25) & ". - [Kabar Terbaru CMC]  " & PenampungBerita
          TampilkanBalon frmMain, j_bahasa(25), i_bahasa(26), NIIF_INFO
       End If
    Else
       CheckUpdate = j_bahasa(24)
       bUpdateCompon = False
       TampilkanBalon frmMain, j_bahasa(24), i_bahasa(27), NIIF_WARNING
    End If
...
With frmMain
    SoftVers(1) = Mid(.lbEngine.Caption, 6) ' PH yang dipakai
...
Dim URLToUpdateCMC As String
Dim iCounter           As Long
Dim TmpSimpan          As String
Dim FolderSign         As String

TmpSimpan = GetSpecFolder(USER_DOC) & "\cmctmp.txt"
FolderSign = GetFilePath(App_FullPathW(False)) & "\sign\"

PB.value = KompNumber
PB.Max = 31
' Update http://www.cmc.codenesia.com/files/update/0x.cmc (0x.cmc adalah file)

URLToUpdateCMC = "http://cmc.codenesia.com/files/update/" & Hex(KompNumber) & "x.cmc"


    If HentikanUpdate = True Then GoTo LBL_BERHENTI
    lblOut.Caption = j_bahasa(30) & " : " & URLToUpdateCMC
    
    InternetGetFile URLToUpdateCMC, TmpSimpan, True
...
TmpSimpan = GetSpecFolder(USER_DOC) & "\cmctmp.txt"
FolderSign = GetFilePath(App_FullPathW(False)) & "\signx\"

PB.value = KompNumber + 15
PB.Max = 31
' Update http://www.cmc.codenesia.com/files/update/0z.cmc (0z.cmc adalah file)

URLToUpdateCMC = "http://cmc.codenesia.com/files/update/" & Hex(KompNumber) & "z.cmc"
