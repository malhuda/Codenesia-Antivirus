...
If ToBeFix > 0 Or HasBeenFix > 0 Then
   If MsgBox(j_bahasa(16) & " " & StrTypeFix & " ?" & Chr(13) & _
          StrTypeFix & " " & j_bahasa(17) & " : " & ToBeFix & Chr(13) & _
          StrTypeFix & " " & j_bahasa(18) & " : " & HasBeenFix, vbInformation + vbYesNo) = vbNo Then
      Exit Function ' ga k setuju keluar
   End If
Else
...
For iCount = 1 To lv.ListItems.count
    ...
    ObjPath = lv.ListItems.Item(iCount).SubItem(2).Text
    StatusObj = lv.ListItems.Item(iCount).SubItem(4).Text
    ...
    If IsFileProtectedBySystem(ObjPath) = False Then ' yakinkan bukan file milik system
       ...
       If StatusObj = "Hidden File" Then
          ...
          lv.ListItems.Item(iCount).SubItem(4).Text = f_bahasa(13)
          lv.ListItems.Item(iCount).Text = Cawang & " - " & Filename
       Else
         'lv.ListItems.Item(iCount).IconIndex = 1
          lv.ListItems.Item(iCount).Cut = False
          lv.ListItems.Item(iCount).SubItem(4).Text = f_bahasa(14)
          lv.ListItems.Item(iCount).Text = Cawang & " - " & Filename
       End If
      DoEvents
    Else
...
Select Case MalwareName
       ...
       'Case "VBS Malscript": LayakDihapus = False - bahasa tidak tetap
